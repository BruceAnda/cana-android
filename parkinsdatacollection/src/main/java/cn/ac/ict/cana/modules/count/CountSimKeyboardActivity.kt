...
import cn.ac.ict.cana.modules.guide.ModelGuideActivity
import cn.ac.ict.cana.modules.guide.ModelGuideActivity2
import cn.ac.ict.cana.modules.upload.UploadActivity
...
open class CountSimKeyboardActivity : YouMengBaseActivity() {
    ...
    private fun insertDB(mark: String) {
        ...
        database.use {
            ...
            values.put(cn.ac.ict.cana.db.bean.History.BATCH, FileUtils.batch)
            values.put(cn.ac.ict.cana.db.bean.History.USERID, Dua.getInstance().currentDuaId)
            values.put(cn.ac.ict.cana.db.bean.History.TYPE, ModuleHelper.MODULE_COUNT)
            values.put(cn.ac.ict.cana.db.bean.History.FILEPATH, FileUtils.filePath)
            values.put(cn.ac.ict.cana.db.bean.History.MARK, mark)
            values.put(cn.ac.ict.cana.db.bean.History.ISUPLOAD, "0")
            // 插入数据库
            insert(cn.ac.ict.cana.db.bean.History.TABLE_NAME, null, values)
        }
    }
